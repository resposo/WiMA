...
class GetSIDs(interfaces.plugins.PluginInterface):
    ...
    def __init__(self, *args, **kwargs):
        vollog.debug("GetSIDs plugin 초기화 중...")
        vollog.debug(f"***************Arguments passed to GetSIDs: args = {args}, kwargs = {kwargs}***************")
        config = kwargs.pop('config', None)
        super().__init__(*args, **kwargs)
        vollog.debug("***************Successfully initialized GetSIDs plugin.***************")
        self.local_system_pids = []
    ...
    def get_local_system_pids(self):
        return self.local_system_pids
            ...
            for sid_string in token.get_sids():
                if sid_string in self.well_known_sids:
                    ...
                    if sid_name_re:
                        ...
                        sid_name = ""
                if isinstance(sid_name, str) and "Local System" in sid_name:
                    self.local_system_pids.append(int(task.UniqueProcessId))
                yield (
                    0,
                    (
    ...
    def run(self):
        try:
            ...
            vollog.debug("Successfully generated the list of SIDs.")
        except Exception as e:
            vollog.error(f"An error occurred while running GetSIDs: {e}")
            return
