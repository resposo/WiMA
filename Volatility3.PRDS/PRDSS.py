...
class CombinedPRDSIAT(interfaces.plugins.PluginInterface):
    ...
    def list_processes(cls, context, layer_name, symbol_table, filter_func=lambda _: False):
        ...
        for proc in eproc.ActiveProcessLinks:
            exe_path = CombinedPRDSIAT.find_exe_path(proc)
            if not filter_func(proc) and "program files/windowsapps" not in exe_path.lower():
                yield proc

    @staticmethod
    def find_exe_path(proc):
        try:
            for module in proc.load_order_modules():
                dll_name = module.FullDllName.get_string()
                if "program files/windowsapps" not in dll_name.lower():
                    return dll_name
        ...
        try:
            ...
            if hasattr(pe_obj, "DIRECTORY_ENTRY_IMPORT"):
                for entry in pe_obj.DIRECTORY_ENTRY_IMPORT:
                    ...
                    if "program files/windowsapps" not in dll_name:
    ...
    def calculate_sha256(self, process_name, exe_path, parent_name, dll_names):
        ...
        # DLL 이름 리스트에서 'Program Files/WindowsApps' 경로 필터링
        filtered_dll_names = [dll for dll in dll_names if "program files/windowsapps" not in dll.lower()]
        
        # DLL 이름 리스트 정렬
        sorted_dll_names = sorted(filtered_dll_names)
    ...
    def check_sha256_in_db(self, sha256, db_path):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM proc_metadata WHERE sha256 = ?", (sha256,))
            ...
            try:
                for row in ldr_output._generator([proc]):
                    ...
                    if isinstance(mapped_path, str):
                        ...
                        if "program files/windowsapps" not in standardized_path.lower():
    ...
    def initialize_db(self, db_path):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS prometadata (
            ...
            ''')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_sha256 ON proc_metadata (sha256)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_pname ON proc_metadata (pname)')
            conn.commit()
            conn.close()
    ...
    def insert_analysis_result(self, db_path, pname, exe_path, ppname, dll_list, sha256):
        ...
        sorted_and_standardized_dll_list = ";".join(sorted(dll_list.replace('\\', '/').split(';'))) if dll_list else ""
        
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO proc_metadata (pname, exe_path, ppname, dll_list, sha256)
            VALUES (?, ?, ?, ?, ?)
    ...
    def find_dll_differences(self, analyzed_dlls, stored_dlls):
        ...
        analyzed_lower = {dll.lower().replace('\\', '/') for dll in analyzed_dlls if "program files/windowsapps" not in dll.lower()}
        stored_lower = set()
    ...
    def get_all_dlls_for_process_name(self, db_path, pname):
        ...
        dll_lists = []
        try:
            cursor.execute("SELECT dll_list FROM proc_metadata WHERE pname = ?", (pname,))
    ...
    def check_process_name_in_db(self, pname, db_path):
        ...
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM proc_metadata WHERE pname = ?", (pname,))
        result = cursor.fetchone()[0] > 0
        conn.close()
        return result
    ...
    def _generator(self):
        ...
        for proc in procs:
            try:
                ...
                dll_list_str = ";".join(sorted(dll_names)) if dll_names else ""
                ...
                if pname_exists_in_db:
                    ...
                    # 저장된 DLL 목록을 가져와 비교하여 실제로 같은지 확인
    ...
    def get_stored_dlls_by_alternate_key(self, db_path, pname, exe_path):
        ...
        try:
            query = """
            SELECT id, dll_list FROM proc_metadata 
    ...
    def run_gui(self):
        ...
        root.protocol("WM_DELETE_WINDOW", app.on_closing)  # 창 닫기 이벤트 핸들러 설정

        # filtered_pids.txt 파일 덮어쓰기 여부 묻기
        if os.path.exists("filtered_pids.txt"):
        ...
        with open("filtered_pids.txt", self.write_mode) as f:
            f.write("\n".join(filtered_pids) + "\n")
...
class CombinedPRDSIAT_GUI:
    ...
    def on_double_click(self, event):
        ...
        pid, pname, _, _, _, _, _, dll_list, _ = item

        # 새로운 창을 생성하여 DLL 목록을 표시
        detail_window = tk.Toplevel(self.master)
        detail_window.title(f"[DFV]PID {pid} - {pname}")

        lbl_dlls = tk.Label(detail_window, text="DLL LIST:")
        lbl_dlls.pack()

        txt_dlls = tk.Text(detail_window, wrap="word")
        txt_dlls.pack(expand=True, fill="both")

        if dll_list:
            for dll in dll_list.split(";"):
                txt_dlls.insert("end", dll + "\n")

        txt_dlls.config(state="disabled")  # 텍스트 박스 비활성화

    def on_closing(self):
        print("Shut down the plug-in.")
        self.master.destroy()
